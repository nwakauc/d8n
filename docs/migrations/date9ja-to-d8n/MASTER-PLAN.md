...
2. **Before writable Date9ja profiles:** complete brand-scoped profile field write enforcement, configuration-derived serialization, and non-sensitive Date9ja catalogue composition. This blocks profile onboarding/editing, profile completion, sensitive-field handling, photos/publication policy integration, and profile API parity; it does not block registering the brand record/contract. — **RESOLVED (2026-09-05, ADR 0030).** `Profiles::FieldCatalog` is now the single canonical definition layer for scalar profile fields; `FieldPolicy`/`Configuration`/`Profile`/`ProfilePreference`/the serializers all derive from it, brand-scoped write enforcement and configuration-derived serialization are fail-closed for every brand, and Date9ja's existing explicit `Date9jaProfileCatalog` was proven end-to-end (real HTTP `PATCH`/`GET`) to require zero Date9ja-specific profile code. This resolves the *platform* blocker only — it does not migrate Date9ja profile values, and it does not resolve the still-open tribe/ethnicity/denomination/genotype/state_of_origin/preferred_tribes/preferred_religion product decisions (`DECISIONS.md`), which remain separately gated. See `STATUS.md` and `CAPABILITY-PARITY.md` for evidence.
