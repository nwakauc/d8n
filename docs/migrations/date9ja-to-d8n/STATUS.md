...
- **2026-09-18 Phase 1 P0 closure — NOT READY / bounded changes SELF_VERIFIED:** Codex owns reference repair,
  stable-binding final synchronization, production-equivalent local rehearsal,
  manifest media transfer, frontend BFF/lifecycle fixes and delivery diagnosis.
  Details: [PHASE1-CLOSURE.md](PHASE1-CLOSURE.md). Raw-source final compiler and
  attachment/evidence graph remain incomplete. Production remains read-only.
  Earlier READY claims are superseded by the
  September 18 audit. Final raw backup, independent review, product retirement
  decisions and production acceptance remain required; no traffic change authorized.

- **Production Closure Pass 2 — migration parity + cutover readiness audit — PARITY_ACCEPTED / CUTOVER READY WITH EXPLICIT OPERATOR STEPS (2026-09-15):** audited every commit since the 2026-09-10 GO-NO-GO report (RealMe verification/moderation, Trust Score System ADR 0025, progressive RealMe gate, genotype/compatibility contract, Community foundation ADR 0033) for migration-parity impact. Found and fixed one real gap: `Date9ja::Import::TrustLedgerImport` was correct and tested but never wired into the operator rake-task chain, so migrated members' trust history would never reach the live Trust Score System — added `date9ja:import_trust_ledger` and inserted it into `PRODUCTION-CUTOVER-RUNBOOK.md`. Confirmed RealMe badge logic cannot be falsely triggered by migrated email/phone verification, and that genotype/Community are unchanged non-gaps. Fixed two stale `faith_family_expectations` test expectations (a new, source-less D8N enrichment field, not a migrated Date9ja concept). Full suite, env-corrected: 2,382 runs / 3 failures / 0 errors, all three confirmed unrelated to Date9ja. No Date9ja snapshot database was available in this pass's sandbox, so snapshot-scale reconciliation numbers remain the 2026-09-10 evidence, unverified against current HEAD (though no migration-path code changed). Full record: [DATE9JA-MIGRATION-PARITY-CLOSURE.md](../../../DATE9JA-MIGRATION-PARITY-CLOSURE.md).

- **Community foundation — IMPLEMENTED / SELF_VERIFIED (2026-09-13, ADR 0033):** Date9ja now enables the shared `community.read`, `community.participation`, and `community.moderation` capabilities. Questions/answers, text stories, events/RSVPs, and approved Circles are brand/profile owned with database composite tenant FKs; public reads require approved, kept, published records; owner edits return approved content to pending review; event rosters are organizer-only; Circle discussion is membership-only. Admin queues use dedicated permissions, mandatory MFA, and content-free audit events. Visible Community records extend the canonical Trust report target registry with bounded evidence. Focused Community and OpenAPI contract tests pass. This is a foundation, not parity acceptance: weekly answer selection, votes, legacy remarks, Community video, notifications, historical import/reconciliation, automated risk flags, frontend compatibility, and the full Date9ja acceptance journey remain open.
...
- **Date9ja compatibility/genotype contract — IMPLEMENTED / SELF-VERIFIED (updated 2026-09-12):** the Date9ja catalogue exposes the legacy cultural path, optional potential-match-visible genotype, six compatibility answer groups, relocation fields, and the legacy 10-character bio gate. `date9ja_v1` provides a thresholded relationship percentage plus a separate hemoglobin-genotype critical check. Missing/not-tested/prefer-not-to-say genotype passes as `not_assessed`; a calculable HbSS, HbSC, or HbCC inherited-disease outcome is omitted from curated daily introductions. No database migration is required. Real-corpus genotype remains absent from sanitized rehearsals and still requires final pristine census measure 327 plus security/DPIA approval before import; frontend presentation and product acceptance remain separate work.
- **Date9ja progressive RealMe gate — IMPLEMENTED / SELF-VERIFIED (2026-09-12, ADR 0031):** identifier confirmation does not block onboarding, discovery, profile detail, Likes/Passes, matching, conversation creation, or message history. Message send requires a verified phone or an approved same-brand phone/selfie/video/liveness/government-ID assertion; verified email alone does not qualify. A future Date9ja message-media intent uses the same rule if that capability is enabled. No new schema migration is required. `IdentityImport` already preserves verified phone and `VerificationImport` preserves legacy assertions, but production verification import/reconciliation, evidence-policy gates, and live non-phone method flows remain outstanding.
...
- **Historical graph/state migration — IMPLEMENTED / SELF-VERIFIED (2026-09-08):** `Date9ja::Snapshot::HistoricalGraphSource` normalizes Date9ja-shaped relationship rows (including deterministic Match→Conversation identities) into `Date9ja::Import::HistoricalGraphImport`, which writes likes, passes (when present), matches, conversations, text messages, blocks, and reports directly to canonical brand-scoped D8N records. Rows resolve only through `Migration::ReferenceMap`; each row is savepointed, fail-closed, idempotently bound, and emits no runtime notification/limit side effects. Profile views remain explicitly deferred pending retention policy. Fixture and adapter verification are complete, and the historical HTTP journey is **fixture-proven through the normal Date9ja runtime** (`test/controllers/api/v1/date9ja_historical_conversation_journey_test.rb`): Date9ja-shaped source rows → snapshot adapter → importer → `GET /api/v1/matches` → `GET /api/v1/conversations` → `GET /conversations/:id/messages` (canonical newest-first order, senders, bodies and historical timestamps intact) → a new native `POST /conversations/:id/messages` → peer read of history plus the new message. Rerunning the import after that native message adds no duplicate match/conversation/message, leaves the native message and every reference binding byte-identical, and emits no `NotificationEvent`. No runtime API or schema change was required and no migration-specific representation is exposed. Real sanitized-corpus rehearsal and snapshot-scale reconciliation remain deferred to whole-system rehearsal; this is not parity acceptance or cutover readiness. Production is untouched.
- Current capability: **Date9ja migrated profile readiness — IMPLEMENTED / SELF-VERIFIED 2026-09-07; ready for feature-boundary review.** The sanitized source rehearsal is explicitly deferred because `DATE9JA_SNAPSHOT_DATABASE_URL` is unavailable; this is evidence pending, not a runtime implementation gap. The migration-only `Date9ja::Import::ProfileReadinessImport` classifies every eligible source member as `ready`, `intentionally_hidden`, `remediation_required`, or `failed`, persists PII-free multi-reason evidence in `Migration::ProfileReadiness`, and fills only deterministic gaps. Exact two-token names map; one-token/3+-token names require confirmation. Reviewed country aliases map to ISO-2; unknowns fail closed. An exact unique Nigerian `Place` may create location through `Profiles::CurrentPlace`; no fuzzy place, invented coordinate, or outside-country fallback is assigned. Missing/partial age ranges and unresolved option codes remain remediation. The default publication policy is `classify_only`; `publish_visible_onboarded` exists for an explicitly approved D-8 run and uses `Profiles::Publication`. Explicitly hidden/suspended members remain hidden. Reruns preserve native values and a member/operator unpublish. **Date9ja Core Dating Loop — IMPLEMENTED / SELF-VERIFIED 2026-09-07:** Date9ja now consumes shared browse discovery, bilateral eligibility, likes, passes, mutual matches, conversations, and text messaging through its BrandContract; historical graph migration remains separate. The native Date9ja HTTP journey is proven, and runtime loop code contains no migrated-user-specific branch. A migrated-member HTTP journey has not been separately exercised.**
- **Current-contract baseline, executed 2026-09-07 against the available isolated identity+photo destination rehearsal (`d8n_date9ja_rehearsal_l2_20260903`) inside a rolled-back contract-refresh transaction:** 280 profiles, **0 complete**. Missing: first name 280, last name 280, country 280, location 280, relationship intent 280, has children 280, wants children 280, interested-in 280, min age 280, max age 280, city 280, bio 74, publication-eligible photo 125. This destination predates the completed Pass-2 preference rehearsal; do not combine these counts with the later 63-age-pair/477-selection evidence. The readiness importer rejects a stale persisted brand contract rather than producing misleading counts.
- Previous programme evidence: the profile/preference census, canonical `man`/`woman` decode, identity/auth transition, and photo/video migration work remain recorded in the dated sections below. Current code maps legacy gender codes correctly; historical text describing raw `"0"`/`"1"` destination values predates the completed Pass-2 importer. **The programme remains NOT PARITY_ACCEPTED, not production-ready, and not cutover-ready; L3 remains outstanding.**
...
- **Date9ja location semantics correction (2026-09-08):** Date9ja asks for
  country and city profile fields only. It does not use device GPS,
  `ProfileLocation` completion, or distance filtering. The shared eligibility
  policy now explicitly disables location filtering for Date9ja; DateZA and
  HookUs policies are unchanged. Historical rehearsal counts below that mention
  a location requirement belong to the earlier contract and are retained as
  historical evidence.
