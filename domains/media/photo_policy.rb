module Media
  ...
  # flags it, moderation flips it back to `hidden`/`rejected`. Any brand
  # without an explicit policy stays moderate-first (`hidden`,
  # `pending_review`) so a new brand never silently publishes unmoderated
  # media. Date9ja therefore keeps the conservative default until it opts in.
  #
