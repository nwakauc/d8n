...
class Api::V1::FindControllerTest < ActionDispatch::IntegrationTest
  setup do
    ...
    identifier = IdentityIdentifier.create!(
      user: @viewer.user, brand: @brand, kind: :email, normalized_value: "find-viewer@example.com", verified_at: Time.current
    )
  ...
  test "a rejected DateZA photo is withheld from Find" do
    candidate = create_candidate
  ...
  test "a pending-review DateZA photo is deliverable in Find results immediately" do
    candidate = create_candidate
    photo = attach_photo(candidate)
